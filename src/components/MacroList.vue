    ...
    <ul class="list-inline">
      <li class="list-inline-item" v-for="(macro, index) in macroList">
        <div class="card mb-2">
          <div class="card-body">
            <h5 class="card-title">
              <span v-if="macro.status==='running'">
                <i class="fa fa-spinner fa-spin"></i> 동작중 ...
              </span>
            ...
            <a
              v-if="macro.status==='running' || macro.status==='error'"
              href="#"
              class="card-link"
              @click="removeMacroRequest(index)"
            >삭제</a>
            <a
              v-if="macro.status==='error'"
              href="#"
              class="card-link pull-right"
              @click="restartMacroRequest(index)"
            >재시작</a>
            <a
              v-if="macro.status==='done'"
